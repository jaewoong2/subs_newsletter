...
const CreatorsLetters = async ({ params }: NextPageProps<Props>) => {
  ...
  return (
    <section className='w-full max-w-2xl px-3'>
      ...
      <div className='flex w-full items-center justify-between rounded-lg bg-slate-100 px-2 py-5 dark:bg-darkBg-200'>
        <p className='text-lg font-semibold max-md:text-base'>{letter?.data.title}</p>
      </div>
      <p className='flex w-full justify-end text-sm font-semibold text-gray-600 dark:text-white max-md:text-xs'>
        {letter?.data.released_at?.slice(2, 10)}
      </p>
