...
const DataList = ({ title, lastItem, isLoading, children, variant = 'carousel' }: PropsWithChildren<Props>) => {
  return (
    <section className='flex h-fit w-full flex-col justify-center'>
      <h3 className='px-20 text-xl font-bold max-lg:px-10 max-md:px-0' id='new'>
        {title}
      </h3>
      {variant === 'carousel' && children}
      {variant === 'block' && (
        <div className='grid h-fit grid-cols-3 px-10 max-lg:grid-cols-2 max-md:grid-cols-2 max-md:px-0'>
          {children}
          {isLoading && (
