...
const Body = ({ title, subTitle, button, buttonVariant = 'normal' }: Props) => {
  return (
    <>
      ...
      >
        <span className='z-10 text-black'>{title}</span>
      </h1>
      <p className='mb-6 max-w-2xl text-gray-600 dark:text-gray-400 md:text-lg lg:mb-8 lg:text-xl'>{subTitle}</p>
      <button
        className={twMerge(
          'btn relative mr-3 inline-flex items-center justify-center rounded-lg px-5 py-3 text-center text-base font-medium hover:text-white focus:ring-4 focus:ring-blue-300',
